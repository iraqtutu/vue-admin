<template>
  <PageWrapper v-loading="cacheState.loading" loading-tip="加载中..." title="UserDep">
    ...
    <div class="item bg-white p-4 m-4" v-for="rec in cacheState.curlist.records" :key="rec.id">
      ...
      <div class="viewid">视图标识:{{ rec.viewId }}</div>
      <div class="departmentid">部门标识:{{ rec.departmentId }}</div>
    ...
    >
      ...
      <div class="viewid">视图标识:{{ cacheState.curRec.viewId }}</div>
      <div class="departmentid">部门标识:{{ cacheState.curRec.departmentId }}</div>
...
<script setup lang="ts">
  ...
  import { queryUserDeps, deleteUserDep } from '/@/api/gen/userdep';
  ...
  function onedit(rec: UserDep) {
    cacheState.curRec = rec;
    cacheState.dialogVisible = true;
  }
  function ondelete(rec: UserDep) {
    deleteUserDep(rec.id).then((res) => {
      if (res) {
        handleCurrentChange(cacheState.currentpage);
      }
    });
