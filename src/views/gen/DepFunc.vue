<template>
  <PageWrapper v-loading="cacheState.loading" loading-tip="加载中..." title="DepFunc">
...
<script setup lang="ts">
  ...
  import { queryDepFuncs, deleteDepFunc } from '/@/api/gen/depfunc';
  ...
  function onedit(rec: DepFunc) {
    cacheState.curRec = rec;
    cacheState.dialogVisible = true;
  }
  function ondelete(rec: DepFunc) {
    deleteDepFunc(rec.id).then((res) => {
      if (res) {
        handleCurrentChange(cacheState.currentpage);
      }
    });
