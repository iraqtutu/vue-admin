<template>
  <PageWrapper v-loading="cacheState.loading" loading-tip="加载中..." title="Department">
    ...
    >
      <span>我是对话框</span>
      <div class="id">部门ID:{{ cacheState.curRec.id }}</div>
      <div class="copid">企业ID:{{ cacheState.curRec.copId }}</div>
      <div class="viewid">视图ID:{{ cacheState.curRec.viewId }}</div>
      <div class="departmentname">部门名称:{{ cacheState.curRec.departmentName }}</div>
      <div class="parentid">上级部门ID:{{ cacheState.curRec.parentId }}</div>
      <div class="extenaljson">部门扩展属性;JSON扩展书属性:{{ cacheState.curRec.extenalJson }}</div>
      <div class="note">部门描述:{{ cacheState.curRec.note }}</div>
...
<script setup lang="ts">
  ...
  const cacheState = reactive({
    ...
    pagesize: 3,
    currentpage: 0,
    curlist: {} as Page<Department>,
    dialogVisible: false,
    curRec: {} as Department,
  });
