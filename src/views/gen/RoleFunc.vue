<template>
  <PageWrapper v-loading="cacheState.loading" loading-tip="加载中..." title="RoleFunc">
...
<script setup lang="ts">
  ...
  import { queryRoleFuncs, deleteRoleFunc } from '/@/api/gen/rolefunc';
  ...
  function onedit(rec: RoleFunc) {
    cacheState.curRec = rec;
    cacheState.dialogVisible = true;
  }
  function ondelete(rec: RoleFunc) {
    deleteRoleFunc(rec.id).then((res) => {
      if (res) {
        handleCurrentChange(cacheState.currentpage);
      }
    });
