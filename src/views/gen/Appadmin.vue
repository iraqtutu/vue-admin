<template>
  <PageWrapper v-loading="cacheState.loading" loading-tip="加载中..." title="Appadmin">
...
<script setup lang="ts">
  ...
  import { queryAppadmins, deleteAppadmin } from '/@/api/gen/appadmin';
  ...
  function onedit(rec: Appadmin) {
    cacheState.curRec = rec;
    cacheState.dialogVisible = true;
  }
  function ondelete(rec: Appadmin) {
    deleteAppadmin(rec.id).then((res) => {
      if (res) {
        handleCurrentChange(cacheState.currentpage);
      }
    });
