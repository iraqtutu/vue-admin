<template>
  <PageWrapper v-loading="cacheState.loading" loading-tip="加载中..." title="DepRole">
    ...
    <div class="item bg-white p-4 m-4" v-for="rec in cacheState.curlist.records" :key="rec.id">
      ...
      <div class="departmentid">部门ID:{{ rec.departmentId }}</div>
      <div class="roleid">角色ID:{{ rec.roleId }}</div>
    ...
    >
      ...
      <div class="departmentid">部门ID:{{ cacheState.curRec.departmentId }}</div>
      <div class="roleid">角色ID:{{ cacheState.curRec.roleId }}</div>
...
<script setup lang="ts">
  ...
  import { queryDepRoles, deleteDepRole } from '/@/api/gen/deprole';
  ...
  function onedit(rec: DepRole) {
    cacheState.curRec = rec;
    cacheState.dialogVisible = true;
  }
  function ondelete(rec: DepRole) {
    deleteDepRole(rec.id).then((res) => {
      if (res) {
        handleCurrentChange(cacheState.currentpage);
      }
    });
