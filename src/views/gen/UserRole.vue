<template>
  <PageWrapper v-loading="cacheState.loading" loading-tip="加载中..." title="UserRole">
...
<script setup lang="ts">
  ...
  import { queryUserRoles, deleteUserRole } from '/@/api/gen/userrole';
  ...
  function onedit(rec: UserRole) {
    cacheState.curRec = rec;
    cacheState.dialogVisible = true;
  }
  function ondelete(rec: UserRole) {
    deleteUserRole(rec.id).then((res) => {
      if (res) {
        handleCurrentChange(cacheState.currentpage);
      }
    });
