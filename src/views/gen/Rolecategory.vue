<template>
  <PageWrapper v-loading="cacheState.loading" loading-tip="加载中..." title="Rolecategory">
...
<script setup lang="ts">
  ...
  import { queryRolecategorys, deleteRolecategory } from '/@/api/gen/rolecategory';
  ...
  const cacheState = reactive({
    ...
    pagesize: 3,
    currentpage: 0,
    curlist: {} as Page<Rolecategory>,
    dialogVisible: false,
    curRec: {} as Rolecategory,
  });
  ...
  function handleCurrentChange(val: number) {
    ...
    queryRolecategorys(param).then((res) => {
  ...
  function onedit(rec: Rolecategory) {
    cacheState.curRec = rec;
    cacheState.dialogVisible = true;
  }
  function ondelete(rec: Rolecategory) {
    deleteRolecategory(rec.id).then((res) => {
      if (res) {
        handleCurrentChange(cacheState.currentpage);
      }
    });
