<template>
  <PageWrapper v-loading="cacheState.loading" loading-tip="加载中..." title="Platconfig">
    ...
    <div class="item bg-white p-4 m-4" v-for="rec in cacheState.curlist.records" :key="rec.id">
      <div class="id">数据唯一标识:{{ rec.id }}</div>
      <div class="varname">参数名:{{ rec.varName }}</div>
      <div class="varvalue">参数值:{{ rec.varValue }}</div>
    ...
    >
      <span>我是对话框</span>
      <div class="id">数据唯一标识:{{ cacheState.curRec.id }}</div>
      <div class="varname">参数名:{{ cacheState.curRec.varName }}</div>
      <div class="varvalue">参数值:{{ cacheState.curRec.varValue }}</div>
...
<script setup lang="ts">
  ...
  import { queryPlatconfigs, deletePlatconfig } from '/@/api/gen/platconfig';
  ...
  const cacheState = reactive({
    ...
    pagesize: 3,
    currentpage: 0,
    curlist: {} as Page<Platconfig>,
    dialogVisible: false,
    curRec: {} as Platconfig,
  });
  ...
  function handleCurrentChange(val: number) {
    ...
    queryPlatconfigs(param).then((res) => {
  ...
  function onedit(rec: Platconfig) {
    cacheState.curRec = rec;
    cacheState.dialogVisible = true;
  }
  function ondelete(rec: Platconfig) {
    deletePlatconfig(rec.id).then((res) => {
      if (res) {
        handleCurrentChange(cacheState.currentpage);
      }
    });
