<template>
  <PageWrapper v-loading="cacheState.loading" loading-tip="加载中..." title="Copadmin">
    ...
    <div class="item bg-white p-4 m-4" v-for="rec in cacheState.curlist.records" :key="rec.id">
      ...
      <div class="copid">入驻企业ID:{{ rec.copId }}</div>
      <div class="userid">登录名:{{ rec.userId }}</div>
    ...
    >
      ...
      <div class="copid">入驻企业ID:{{ cacheState.curRec.copId }}</div>
      <div class="userid">登录名:{{ cacheState.curRec.userId }}</div>
...
<script setup lang="ts">
  ...
  import { queryCopadmins, deleteCopadmin } from '/@/api/gen/copadmin';
  ...
  function onedit(rec: Copadmin) {
    cacheState.curRec = rec;
    cacheState.dialogVisible = true;
  }
  function ondelete(rec: Copadmin) {
    deleteCopadmin(rec.id).then((res) => {
      if (res) {
        handleCurrentChange(cacheState.currentpage);
      }
    });
